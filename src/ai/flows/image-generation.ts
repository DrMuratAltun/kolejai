...
const generateImageFlow = ai.defineFlow(
  ...
  async ({ prompt }) => {
    const {media} = await ai.generate({
      ...
      prompt: `A high quality, photorealistic image of: ${prompt}. The image must not contain any text, writing, or letters. Cinematic lighting. Wide-angle shot, aspect ratio 16:9.`,
      config: {
